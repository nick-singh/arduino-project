...
if (Serial.available()){
     ...
     if(Serial.read() == 116){ //if we get t as input
       SendTextMessage();
     }
     if(Serial.read() == 104){ // if we get h as
       SubmitHttpRequest();
     }  
...
delay(100);
mySerial.println("AT + CMGS = \"+8687343346\"");//send sms message, be careful need to add a country code before the cellphone number
delay(100);
